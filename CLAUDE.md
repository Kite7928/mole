...
## ⚠️ API 速率控制（最高优先级）
当前 API 代理限速 5次/分钟，必须严格遵守以下规则：
- **绝对禁止**并行启动多个 Task 子代理（一次只能有一个 Task）
- **绝对禁止**在同一条消息中发起超过 2 个工具调用
- **优先使用本地工具**：Read、Glob、Grep、Edit、Write 不消耗 API 配额，必须优先使用
- **禁止使用 WebSearch、WebFetch**：这些工具会额外消耗 API 请求
- **Task 子代理使用 haiku 模型**：降低资源消耗，加快响应
- 遇到 429 错误时，等待 30 秒后重试，不要连续重试
...
### 📋 编码前规范（新增规则）
- **方案先行**：编码前必须先描述详细方案，等待用户批准后方可开始编码。
- **需求澄清**：需求不明确时必须提出澄清问题，确保完全理解需求后再开始工作。
- **任务分解**：涉及修改超过3个文件的任务，必须先分解成更小的独立单元，分步骤执行。

### 🧪 编码后规范（新增规则）
- **质量保证**：代码完成后必须列出潜在问题并提供测试用例。
- **Bug修复流程**：发现bug时必须先编写复现测试，修复至测试通过后方可提交。
- **规则迭代**：每次被用户纠正后，必须在 CLAUDE.md 中添加新规则，避免重复犯相同错误。
