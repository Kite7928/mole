...
class TestNewsFetcherService:
    ...
    @pytest.mark.asyncio
    async def test_fetch_from_ithome_with_category_filter(self, news_fetcher, mock_rss_feed):
        ...
        with patch('feedparser.parse', return_value=mock_rss_feed):
            ...
            result = await news_fetcher.fetch_news(
                NewsSource.ITHOME,
                limit=10,
                category_filter=NewsCategory.ENTERTAINMENT
            )

            # Should return empty since IT_HOME is TECH category
            assert len(result) == 0
